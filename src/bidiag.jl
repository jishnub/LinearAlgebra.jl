...
function _bibimul!(C, A, B, alpha, beta)
    ...
    if n <= 3
        ...
        for I in CartesianIndices(C)
            C[I] += @stable_muladdmul MulAddMul(alpha,beta)(sum(A[I[1], k] * B[k, I[2]] for k in axes(A,2)))
        end
        return C
    end
    @inbounds begin
        # first column of C
        C[1,1] += @stable_muladdmul MulAddMul(alpha,beta)(A[1,1]*B[1,1] + A[1, 2]*B[2,1])
        C[2,1] += @stable_muladdmul MulAddMul(alpha,beta)(A[2,1]*B[1,1] + A[2,2]*B[2,1])
        C[3,1] += @stable_muladdmul MulAddMul(alpha,beta)(A[3,2]*B[2,1])
        # second column of C
        C[1,2] += @stable_muladdmul MulAddMul(alpha,beta)(A[1,1]*B[1,2] + A[1,2]*B[2,2])
        C[2,2] += @stable_muladdmul MulAddMul(alpha,beta)(A[2,1]*B[1,2] + A[2,2]*B[2,2] + A[2,3]*B[3,2])
        C[3,2] += @stable_muladdmul MulAddMul(alpha,beta)(A[3,2]*B[2,2] + A[3,3]*B[3,2])
        C[4,2] += @stable_muladdmul MulAddMul(alpha,beta)(A[4,3]*B[3,2])
    end # inbounds
    # middle columns
    __bibimul!(C, A, B, alpha, beta)
    @inbounds begin
        C[n-3,n-1] += @stable_muladdmul MulAddMul(alpha,beta)(A[n-3,n-2]*B[n-2,n-1])
        C[n-2,n-1] += @stable_muladdmul MulAddMul(alpha,beta)(A[n-2,n-2]*B[n-2,n-1] + A[n-2,n-1]*B[n-1,n-1])
        C[n-1,n-1] += @stable_muladdmul MulAddMul(alpha,beta)(A[n-1,n-2]*B[n-2,n-1] + A[n-1,n-1]*B[n-1,n-1] + A[n-1,n]*B[n,n-1])
        C[n,  n-1] += @stable_muladdmul MulAddMul(alpha,beta)(A[n,n-1]*B[n-1,n-1] + A[n,n]*B[n,n-1])
        # last column of C
        C[n-2,  n] += @stable_muladdmul MulAddMul(alpha,beta)(A[n-2,n-1]*B[n-1,n])
        C[n-1,  n] += @stable_muladdmul MulAddMul(alpha,beta)(A[n-1,n-1]*B[n-1,n  ] + A[n-1,n]*B[n,n  ])
        C[n,    n] += @stable_muladdmul MulAddMul(alpha,beta)(A[n,n-1]*B[n-1,n  ] + A[n,n]*B[n,n  ])
    end # inbounds
    C
end
function __bibimul!(C, A, B, alpha, beta)
    ...
    @inbounds begin
        for j in 3:n-2
            ...
            C[j-2, j] += @stable_muladdmul MulAddMul(alpha,beta)(Aj₋2j₋1*Bj₋1j)
            C[j-1, j] += @stable_muladdmul MulAddMul(alpha,beta)(Aj₋1j₋1*Bj₋1j + Aj₋1j*Bjj)
            C[j,   j] += @stable_muladdmul MulAddMul(alpha,beta)(Ajj₋1*Bj₋1j + Ajj*Bjj + Ajj₊1*Bj₊1j)
            C[j+1, j] += @stable_muladdmul MulAddMul(alpha,beta)(Aj₊1j*Bjj + Aj₊1j₊1*Bj₊1j)
            C[j+2, j] += @stable_muladdmul MulAddMul(alpha,beta)(Aj₊2j₊1*Bj₊1j)
        end
    end
    C
    ...
    if B.uplo == 'U'
        ...
        @inbounds begin
            for j in 3:n-2
                ...
                C[j-2, j] += @stable_muladdmul MulAddMul(alpha,beta)(Aj₋2j₋1*Bj₋1j)
                C[j-1, j] += @stable_muladdmul MulAddMul(alpha,beta)(Aj₋1j₋1*Bj₋1j + Aj₋1j*Bjj)
                C[j,   j] += @stable_muladdmul MulAddMul(alpha,beta)(Ajj₋1*Bj₋1j + Ajj*Bjj)
                C[j+1, j] += @stable_muladdmul MulAddMul(alpha,beta)(Aj₊1j*Bjj)
            end
        end
    else # B.uplo == 'L'
        Bl = _diag(B, -1)
        @inbounds begin
            for j in 3:n-2
                ...
                C[j-1, j] += @stable_muladdmul MulAddMul(alpha,beta)(Aj₋1j*Bjj)
                C[j,   j] += @stable_muladdmul MulAddMul(alpha,beta)(Ajj*Bjj + Ajj₊1*Bj₊1j)
                C[j+1, j] += @stable_muladdmul MulAddMul(alpha,beta)(Aj₊1j*Bjj + Aj₊1j₊1*Bj₊1j)
                C[j+2, j] += @stable_muladdmul MulAddMul(alpha,beta)(Aj₊2j₊1*Bj₊1j)
            end
        end
    ...
    if A.uplo == 'U'
        ...
        @inbounds begin
            for j in 3:n-2
                ...
                C[j-1, j] += @stable_muladdmul MulAddMul(alpha,beta)(Aj₋1j₋1*Bj₋1j + Aj₋1j*Bjj)
                C[j,   j] += @stable_muladdmul MulAddMul(alpha,beta)(Ajj*Bjj       + Ajj₊1*Bj₊1j)
                C[j+1, j] += @stable_muladdmul MulAddMul(alpha,beta)(Aj₊1j₊1*Bj₊1j)
            end
        end
    else # A.uplo == 'L'
        Al = _diag(A, -1)
    ...
    if A.uplo == 'U' && B.uplo == 'U'
        ...
        @inbounds begin
            for j in 3:n-2
                ...
                C[j-2, j] += @stable_muladdmul MulAddMul(alpha,beta)(Aj₋2j₋1*Bj₋1j)
                C[j-1, j] += @stable_muladdmul MulAddMul(alpha,beta)(Aj₋1j₋1*Bj₋1j + Aj₋1j*Bjj)
                C[j,   j] += @stable_muladdmul MulAddMul(alpha,beta)(Ajj*Bjj)
            end
        end
...
function _mul!(C::AbstractMatrix, A::BiTriSym, B::Diagonal, alpha::Number, beta::Number)
    ...
    @inbounds begin
        # first row of C
        for j in 1:min(2, n)
            C[1,j] += @stable_muladdmul MulAddMul(alpha,beta)(A[1,j]*B[j,j])
        end
        # second row of C
        if n > 1
            for j in 1:min(3, n)
                C[2,j] += @stable_muladdmul MulAddMul(alpha,beta)(A[2,j]*B[j,j])
            end
        end
        for j in 3:n-2
            C[j, j-1] += @stable_muladdmul MulAddMul(alpha,beta)(Al[j-1]*Bd[j-1])
            C[j, j  ] += @stable_muladdmul MulAddMul(alpha,beta)(Ad[j  ]*Bd[j  ])
            C[j, j+1] += @stable_muladdmul MulAddMul(alpha,beta)(Au[j  ]*Bd[j+1])
        end
        if n > 3
            # row before last of C
            for j in n-2:n
                C[n-1,j] += @stable_muladdmul MulAddMul(alpha,beta)(A[n-1,j]*B[j,j])
            end
        end
        # last row of C
        if n > 2
            for j in n-1:n
                C[n,j] += @stable_muladdmul MulAddMul(alpha,beta)(A[n,j]*B[j,j])
            end
        end
    end # inbounds
    C
...
function _mul!(C::AbstractMatrix, A::Bidiagonal, B::Diagonal, alpha::Number, beta::Number)
    ...
    @inbounds begin
        # first row of C
        C[1,1] += @stable_muladdmul MulAddMul(alpha,beta)(dv[1]*Bd[1])
        if n > 1
            if A.uplo == 'L'
                C[2,1] += @stable_muladdmul MulAddMul(alpha,beta)(ev[1]*Bd[1])
            end
            for col in 2:n-1
                C[col+rowshift, col] += @stable_muladdmul MulAddMul(alpha,beta)(ev[col - evshift]*Bd[col])
                C[col, col] += @stable_muladdmul MulAddMul(alpha,beta)(dv[col]*Bd[col])
            end
            if A.uplo == 'U'
                C[n-1,n] += @stable_muladdmul MulAddMul(alpha,beta)(ev[n-1]*Bd[n])
            end
            C[n, n] += @stable_muladdmul MulAddMul(alpha,beta)(dv[n]*Bd[n])
        end
    end # inbounds
    C
end
...
function _mul!(C::AbstractVecOrMat, A::BiTriSym, B::AbstractVecOrMat, alpha::Number, beta::Number)
    ...
    _mul_bitrisym!(C, A, B, alpha, beta)
end
function _mul_bitrisym!(C::AbstractVecOrMat, A::Bidiagonal, B::AbstractVecOrMat, alpha::Number, beta::Number)
    nA = size(A,1)
    nB = size(B,2)
    d = A.dv
    if A.uplo == 'U'
...
function _mul_bitrisym!(C::AbstractVecOrMat, A::TriSym, B::AbstractVecOrMat, alpha::Number, beta::Number)
    nA = size(A,1)
    nB = size(B,2)
    l = _diag(A, -1)
    d = _diag(A, 0)
...
function _mul!(C::AbstractMatrix, A::AbstractMatrix, B::TriSym, alpha::Number, beta::Number)
...
function _mul!(C::AbstractMatrix, A::AbstractMatrix, B::Bidiagonal, alpha::Number, beta::Number)
    require_one_based_indexing(C, A)
    matmul_size_check(size(C), size(A), size(B))
    m, n = size(A)
...
#     @stable_muladdmul _mul!(C, A, B, MulAddMul(alpha, beta))
_mul!(C::AbstractMatrix, A::Diagonal, B::Bidiagonal, alpha::Number, beta::Number) =
    _dibimul!(C, A, B, alpha, beta)
_mul!(C::AbstractMatrix, A::Diagonal, B::TriSym, alpha::Number, beta::Number) =
    _dibimul!(C, A, B, alpha, beta)
function _dibimul!(C, A, B, alpha, beta)
    ...
    if n <= 3
        ...
        for I in CartesianIndices(C)
            C[I] += @stable_muladdmul MulAddMul(alpha,beta)(A.diag[I[1]] * B[I[1], I[2]])
        end
        return C
    end
    Ad = A.diag
    ...
    @inbounds begin
        # first row of C
        C[1,1] += @stable_muladdmul MulAddMul(alpha,beta)(A[1,1]*B[1,1])
        C[1,2] += @stable_muladdmul MulAddMul(alpha,beta)(A[1,1]*B[1,2])
        # second row of C
        C[2,1] += @stable_muladdmul MulAddMul(alpha,beta)(A[2,2]*B[2,1])
        C[2,2] += @stable_muladdmul MulAddMul(alpha,beta)(A[2,2]*B[2,2])
        C[2,3] += @stable_muladdmul MulAddMul(alpha,beta)(A[2,2]*B[2,3])
        for j in 3:n-2
            Ajj       = Ad[j]
            C[j, j-1] += @stable_muladdmul MulAddMul(alpha,beta)(Ajj*Bl[j-1])
            C[j, j  ] += @stable_muladdmul MulAddMul(alpha,beta)(Ajj*Bd[j])
            C[j, j+1] += @stable_muladdmul MulAddMul(alpha,beta)(Ajj*Bu[j])
        end
        # row before last of C
        C[n-1,n-2] += @stable_muladdmul MulAddMul(alpha,beta)(A[n-1,n-1]*B[n-1,n-2])
        C[n-1,n-1] += @stable_muladdmul MulAddMul(alpha,beta)(A[n-1,n-1]*B[n-1,n-1])
        C[n-1,n  ] += @stable_muladdmul MulAddMul(alpha,beta)(A[n-1,n-1]*B[n-1,n  ])
        # last row of C
        C[n,n-1] += @stable_muladdmul MulAddMul(alpha,beta)(A[n,n]*B[n,n-1])
        C[n,n  ] += @stable_muladdmul MulAddMul(alpha,beta)(A[n,n]*B[n,n  ])
    end # inbounds
    C
end
function _dibimul!(C::AbstractMatrix, A::Diagonal, B::Bidiagonal, alpha::Number, beta::Number)
    require_one_based_indexing(C)
    matmul_size_check(size(C), size(A), size(B))
    n = size(A,1)
    iszero(n) && return C
    ...
    @inbounds begin
        # first row of C
        C[1,1] += @stable_muladdmul MulAddMul(alpha,beta)(Ad[1]*Bdv[1])
        if n > 1
            if B.uplo == 'L'
                C[2,1] += @stable_muladdmul MulAddMul(alpha,beta)(Ad[2]*Bev[1])
            end
            for col in 2:n-1
                evrow = col+rowshift
                C[evrow, col] += @stable_muladdmul MulAddMul(alpha,beta)(Ad[evrow]*Bev[col - evshift])
                C[col, col] += @stable_muladdmul MulAddMul(alpha,beta)(Ad[col]*Bdv[col])
            end
            if B.uplo == 'U'
                C[n-1,n] += @stable_muladdmul MulAddMul(alpha,beta)(Ad[n-1]*Bev[n-1])
            end
            C[n, n] += @stable_muladdmul MulAddMul(alpha,beta)(Ad[n]*Bdv[n])
        end
    end # inbounds
    C
end
function _dibimul!(C::Bidiagonal, A::Diagonal, B::Bidiagonal, alpha::Number, beta::Number)
    matmul_size_check(size(C), size(A), size(B))
    n = size(A,1)
    n == 0 && return C
    iszero(alpha) && return _rmul_or_fill!(C, beta)
